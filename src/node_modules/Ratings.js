        ...
        yield Sql.using(
            function * (db) {
                var allGames = yield db.query(
                                    'SELECT G.created_date, G.game_type_id, GP.* '+
                                    'FROM games G '+
                ...
                var lastRecalc = allGames.rows[0].created_date;
                var pendingMatches = [];
                ...
                for(let i = 0; i < allGames.rowCount; i+=2) {
                    ...
                    var needUpdate = (gp1.created_date - lastRecalc) >= intervalMilliseconds;
                    if(needUpdate) {
                        processPendingMatches(gp1.created_date);
                    }
