...
Player.getUrl = function*(id)
{
	var player = yield Player.getById(id);
	var slug = player.name.split(" ").join("-").toLowerCase();
	var url = "players/" + id + "/" + slug;
...
Player.prototype.save = function * ()
{
	...
	{
		var sql, res;
		var params = [ this.name, this.email, this.affiliation, this.rating, this.googleId, this.googleProfile, this.imageUrl ];
		if (!this.id)
		{
			// insert
			sql = 'insert into players (name, email, affiliation, rating, google_id, google_profile, image_url) values ($1, $2, $3, $4, $5, $6, $7) returning id;';
		...
		// update
		sql = 'update players set name=$1, email=$2, affiliation=$3, rating=$4, google_id=$5, google_profile=$6, image_url=$7 where id = $7';
		params.push(this.id);
		res = yield db.query(sql, params);
