...
Game.getById = function*(id)
{
	...
	yield Sql.using(function*(db) 
	{
		...
		var dbResult = yield db.query(sp.sql, sp.params);
...
*/
	...
	var params = [],
		updateSql = function(search, clause, param)
		{
			if (param)
			{
				// postgres doesn't support named parameters?
	...
	updateSql('--top', 'top', settings.take);
	updateSql('--where', 'where g.id =', settings.gameId);
	updateSql('--order by', 'order by', settings.sort);
