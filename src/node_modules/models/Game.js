...
Game.getMany = function*(options)
{
	var defaults = 
		{ 
			...
			orderBy: null	// how games should be ordered in the result, e.g. 'g.created_date desc' for recent games
		},
...
Game.getRecent = function*(limit)
{
	...
	return yield Game.getMany({ limit: limit || 40,  orderBy: 'g.created_date desc, g.id desc' }); 
};

/**
 * returns true if the same players played in both games on the same teams, 
 * regardless of who won (remember, team 1 is the winner of any game)
 * @param game1 {Game}
 * @param game2 {Game}
 */
Game.hasSameTeams = function(game1, game2)
{
	var isSame = function(game1Team)
	{
		return Game.isSameTeam(game1Team, game2.team1) || Game.isSameTeam(game1Team, game2.team2);
	};
	return isSame(game1.team1) && isSame(game1.team2);	
};

/**
 * returns true if the two player arrays consist of the same players
 * @param {Array} teamArray1 an array of {Player} objects
 * @param {Array} teamArray2 an array of {Player} objects
 */
Game.isSameTeam = function (teamArray1, teamArray2)
{
	if (teamArray1.length === 0 || teamArray2.length === 0) return false;
	if (teamArray1.length !== teamArray2.length) return false;

	for (var i = 0; i < teamArray1.length; i++)
	{
		if (teamArray1[i].id != teamArray2[i].id) return false;
	}
...
function parseGameFromQuery (rows)
{
	...
	for (var i = 0; i < rows.length; i++)
	{
		...
		{
			g.id = r.game_id;
			g.created_date = r.game_created_date;
			g.game_type_id = rows.length / 2;
			g.match_id = r.match_id;
			g.created_by_player_id = r.created_by_player_id;
		}
		var p = new Player(r);
	...
	// doubles teams need to sort their players alphabetically
	if (g.team1.length > 1)
	{
		var sortPlayers = function (teamArray)
		{
			teamArray.sort(function(p1, p2) { return p1.name.localeCompare(p2.name); });
		};
		sortPlayers(g.team1);
		sortPlayers(g.team2);
	}
...
function* insertGame(game)
{
	yield Sql.using(function*(db)
	{
		...
		var dbResult = yield db.query(
			"insert into games (created_date, game_type_id, created_by_player_id) values ($1, $2, $3) returning id;",
			[game.created_date, game.game_type_id, game.created_by_player_id]);
...
function getSqlAndParams(options)
{
	...
	// TODO: create something like dapper's multi-mapper, so we don't have to alias columns with idential names
	// see https://github.com/StackExchange/dapper-dot-net#multi-mapping
...
select 
	g.id as game_id,
	g.created_date as game_created_date,
	g.created_by_player_id,
	g.match_id,
	p.id as player_id,
	p.player_type_id,
	p.created_date as player_created_date,
...
function validateBeforeSave(game)
{
	...
	if (!game.created_by_player_id)
		return 'created_by_player_id is required';
