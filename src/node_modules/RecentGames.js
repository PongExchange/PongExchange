...
RecentGames.getRecentGames = function*(recentGamesLimit)
{
	...
	yield Sql.using(function*(db)
	{
		var allGames = yield db.query(
			'SELECT G.played_date, G.game_type_id, G.recorded_by_player_id, GP.* '+
			'FROM games G '+
		...
		var game = [];
		var games = [];
		var current_game_id = allGames.rows[0].game_id;

		for(var i = 0; i<allGames.rowCount; i+=1) 
		{
			if (allGames.rows[i].game_id === current_game_id)
			{
				game.push(allGames.rows[i]);
			} 
			else if (allGames.rows[i].game_id !== current_game_id)
			{
				games.push(game);
				current_game_id-=1;
				game = [];
